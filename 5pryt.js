document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('dato').addEventListener('submit', function(event) {
        ...
        // Resaltar el cuadro con el menor número de baterías
        const resultados = [r12V, r24V, r48V];
        const menorCantidadBaterias = Math.min(...resultados.map(resultado => resultado.cantidadBaterias));
        resultados.forEach(resultado => {
            const elemento = document.getElementById(`result_${resultado.voltaje}V`);
            if (resultado.cantidadBaterias === menorCantidadBaterias) {
